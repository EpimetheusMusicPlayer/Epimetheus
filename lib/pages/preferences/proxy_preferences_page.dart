...
class _ProxyConfigurationUIState extends State<_ProxyConfigurationUI> {
  ...
  SecureStorageManager _secureStorageManager;
  ...
  Future<void> _updateProxyProvider() async {
    if (_secureStorageManager == null) _secureStorageManager = await getPlatformSecureStorageManager();
    _proxyProvider = _selectedProxyId != _noProxyId
        ? ProxyManager.getProxyProviderFromId(
            widget.prefs,
  ...
  @override
  Widget build(BuildContext context) {
    return _secureStorageManager == null
        ? const SizedBox()
        : Padding(
